...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    $jobid = $_POST['jobid'];
    $sql = "SELECT * FROM bid WHERE job_id = '$jobid'";
    ...
    <div class="container">
        <div class="row">
          ...
          <div style="background-color: #f9f9f9; padding: 4%; border-bottom: 1px solid #c3c3c3;">
            ...
            <h4 class="text-left">Bidder In Your Job</h4>
          </div>
        ...
        if($rs)
        {
            ...
            {
                $bid_id=$row['bid_id'];
                echo "Bid By : ".$row['bid_by']."<br>";
                echo "Bid Amount : ".$row['bid_amount']."<br>";
                echo "Bid Duration : ".$row['bid_duration']."<br>";
                //echo "Bid Amount : "$row['bid_status'];

                
                echo "<form action='activeBidStatus.php' method='POST'>";
                echo "<input type='hidden' name='bidid' value='".$bid_id."'>";
                echo "<input type='hidden' name='amount' value='".$row['bid_amount']."'>";
                ...
                echo "<textarea id='msgtext' rows='4' cols='50' name='m' class='form-control' ></textarea> ";
                echo "<input type='hidden' name='p' value='".$row['bid_by']."'>";
                echo "<input type='hidden' name='u' value='".$user."'>";
                echo "<input type='submit' class='btn btn-success' value='Send Messages'>";
                echo "</form>";
                $i++;
            }
        }
...
$sql = "SELECT  name, dob, jobdone, skill, rate, dp, country,balance FROM freelancer where username='$user'";
...
if ($result->num_rows > 0) {
    ...
    while(($row = $result->fetch_assoc()) ) {
        ...
        <p>Job Done : <span style='float:right;'>"  . $row["jobdone"]. "</span></p> <p>Balance : <span style='float:right;'>$"  . $row["balance"]. "</span></p></p></div> ";
        $count_u++;
          echo "<br>";
    }
} else {
